...
interface GardenofficeandshedPageProps {
  params: {
    location?: string[]
  }
}

export async function generateMetadata({ params }: GardenofficeandshedPageProps): Promise<Metadata> {
  const location = params.location?.[0] || 'uk'
  const displayLocation = location.replace(/-/g, ' ')
  const displayKeyword = 'garden-office-and-shed'.replace(/-/g, ' ')
...
export default function GardenofficeandshedPage({ params }: GardenofficeandshedPageProps) {
  const location = params.location?.[0] || 'uk'
  
  // Redirect to search page with proper parameters
  redirect(`/search?type=garden-office-and-shed&location=${location}`)
}
