...
interface GardenofficepodcheapPageProps {
  params: {
    location?: string[]
  }
}

export async function generateMetadata({ params }: GardenofficepodcheapPageProps): Promise<Metadata> {
  const location = params.location?.[0] || 'uk'
  const displayLocation = location.replace(/-/g, ' ')
  const displayKeyword = 'garden-office-pod-cheap'.replace(/-/g, ' ')
...
export default function GardenofficepodcheapPage({ params }: GardenofficepodcheapPageProps) {
  const location = params.location?.[0] || 'uk'
  
  // Redirect to search page with proper parameters
  redirect(`/search?type=garden-office-pod-cheap&location=${location}`)
}
