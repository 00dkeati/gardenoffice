...
interface GardenofficeandstorageshedPageProps {
  params: {
    location?: string[]
  }
}

export async function generateMetadata({ params }: GardenofficeandstorageshedPageProps): Promise<Metadata> {
  const location = params.location?.[0] || 'uk'
  const displayLocation = location.replace(/-/g, ' ')
  const displayKeyword = 'garden-office-and-storage-shed'.replace(/-/g, ' ')
...
export default function GardenofficeandstorageshedPage({ params }: GardenofficeandstorageshedPageProps) {
  const location = params.location?.[0] || 'uk'
  
  // Redirect to search page with proper parameters
  redirect(`/search?type=garden-office-and-storage-shed&location=${location}`)
}
