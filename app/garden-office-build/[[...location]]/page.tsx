...
interface GardenofficebuildPageProps {
  params: {
    location?: string[]
  }
}

export async function generateMetadata({ params }: GardenofficebuildPageProps): Promise<Metadata> {
  const location = params.location?.[0] || 'uk'
  const displayLocation = location.replace(/-/g, ' ')
  const displayKeyword = 'garden-office-build'.replace(/-/g, ' ')
...
export default function GardenofficebuildPage({ params }: GardenofficebuildPageProps) {
  const location = params.location?.[0] || 'uk'
  
  // Redirect to search page with proper parameters
  redirect(`/search?type=garden-office-build&location=${location}`)
}
