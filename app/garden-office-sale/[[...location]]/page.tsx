...
interface GardenofficesalePageProps {
  params: {
    location?: string[]
  }
}

export async function generateMetadata({ params }: GardenofficesalePageProps): Promise<Metadata> {
  const location = params.location?.[0] || 'uk'
  const displayLocation = location.replace(/-/g, ' ')
  const displayKeyword = 'garden-office-sale'.replace(/-/g, ' ')
...
export default function GardenofficesalePage({ params }: GardenofficesalePageProps) {
  const location = params.location?.[0] || 'uk'
  
  // Redirect to search page with proper parameters
  redirect(`/search?type=garden-office-sale&location=${location}`)
}
