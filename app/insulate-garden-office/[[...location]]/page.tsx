...
interface InsulategardenofficePageProps {
  params: {
    location?: string[]
  }
}

export async function generateMetadata({ params }: InsulategardenofficePageProps): Promise<Metadata> {
  const location = params.location?.[0] || 'uk'
  const displayLocation = location.replace(/-/g, ' ')
  const displayKeyword = 'insulate-garden-office'.replace(/-/g, ' ')
...
export default function InsulategardenofficePage({ params }: InsulategardenofficePageProps) {
  const location = params.location?.[0] || 'uk'
  
  // Redirect to search page with proper parameters
  redirect(`/search?type=insulate-garden-office&location=${location}`)
}
