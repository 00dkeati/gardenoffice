...
interface GardenofficeroomsPageProps {
  params: {
    location?: string[]
  }
}

export async function generateMetadata({ params }: GardenofficeroomsPageProps): Promise<Metadata> {
  const location = params.location?.[0] || 'uk'
  const displayLocation = location.replace(/-/g, ' ')
  const displayKeyword = 'garden-office-rooms'.replace(/-/g, ' ')
...
export default function GardenofficeroomsPage({ params }: GardenofficeroomsPageProps) {
  const location = params.location?.[0] || 'uk'
  
  // Redirect to search page with proper parameters
  redirect(`/search?type=garden-office-rooms&location=${location}`)
}
