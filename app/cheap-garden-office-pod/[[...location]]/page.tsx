...
interface CheapgardenofficepodPageProps {
  params: {
    location?: string[]
  }
}

export async function generateMetadata({ params }: CheapgardenofficepodPageProps): Promise<Metadata> {
  const location = params.location?.[0] || 'uk'
  const displayLocation = location.replace(/-/g, ' ')
  const displayKeyword = 'cheap-garden-office-pod'.replace(/-/g, ' ')
...
export default function CheapgardenofficepodPage({ params }: CheapgardenofficepodPageProps) {
  const location = params.location?.[0] || 'uk'
  
  // Redirect to search page with proper parameters
  redirect(`/search?type=cheap-garden-office-pod&location=${location}`)
}
