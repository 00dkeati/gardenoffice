...
interface GardenofficebuildingsPageProps {
  params: {
    location?: string[]
  }
}

export async function generateMetadata({ params }: GardenofficebuildingsPageProps): Promise<Metadata> {
  const location = params.location?.[0] || 'uk'
  const displayLocation = location.replace(/-/g, ' ')
  const displayKeyword = 'garden-office-buildings'.replace(/-/g, ' ')
...
export default function GardenofficebuildingsPage({ params }: GardenofficebuildingsPageProps) {
  const location = params.location?.[0] || 'uk'
  
  // Redirect to search page with proper parameters
  redirect(`/search?type=garden-office-buildings&location=${location}`)
}
