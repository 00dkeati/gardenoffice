...
interface UkgardenofficePageProps {
  params: {
    location?: string[]
  }
}

export async function generateMetadata({ params }: UkgardenofficePageProps): Promise<Metadata> {
  const location = params.location?.[0] || 'uk'
  const displayLocation = location.replace(/-/g, ' ')
  const displayKeyword = 'uk-garden-office'.replace(/-/g, ' ')
...
export default function UkgardenofficePage({ params }: UkgardenofficePageProps) {
  const location = params.location?.[0] || 'uk'
  
  // Redirect to search page with proper parameters
  redirect(`/search?type=uk-garden-office&location=${location}`)
}
