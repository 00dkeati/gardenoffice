...
interface GardenofficeroomPageProps {
  params: {
    location?: string[]
  }
}

export async function generateMetadata({ params }: GardenofficeroomPageProps): Promise<Metadata> {
  const location = params.location?.[0] || 'uk'
  const displayLocation = location.replace(/-/g, ' ')
  const displayKeyword = 'garden-office-room'.replace(/-/g, ' ')
...
export default function GardenofficeroomPage({ params }: GardenofficeroomPageProps) {
  const location = params.location?.[0] || 'uk'
  
  // Redirect to search page with proper parameters
  redirect(`/search?type=garden-office-room&location=${location}`)
}
